...
#define UNSEENWORDPROB                      0.00001
#define UNSEENTRANSITIONPROB                0.00000001    
// Note: UNSEENTRANSITIONPROB will only be used to cancel out the zero probabilities of unsmoothed models. 
...
private: // Typedefs
    ...
    typedef std::vector<StringVector>         StringVectorVector;   ///< Vector of string vectors
    ...
    const std::string viterbi(const std::string& file) {
        ...
        if (in) {
            ...
            for (unsigned i = 0; i < N; ++i) {
                ...
                // and only get zero probabilities. Because of this, use constant UNSEENWORDPROB for multiplication instead.
                if (get_word_index(observations[0]) == -1) {
                    viterbi(i, 0) = init_probs[i] * UNSEENWORDPROB;
                }
                    ...
                    //Handle all possible cases of zero probs (only necessary for unsmoothed models)
                    if (init_probs[i] == 0 && current_observation_prob == 0) {
                        viterbi(i, 0) = UNSEENTRANSITIONPROB * UNSEENTRANSITIONPROB;
                    }
                    else if (init_probs[i] == 0 && current_observation_prob != 0) {
                        viterbi(i, 0) = UNSEENTRANSITIONPROB * current_observation_prob;
                    }
                    else if (init_probs[i] != 0 && current_observation_prob == 0) {
                        viterbi(i, 0) = init_probs[i] * UNSEENTRANSITIONPROB;
                    }
                    else {  // if the model is smoothed, the actual probabilities will be used
                        viterbi(i, 0) = init_probs[i] * current_observation_prob;
                    }
                }
            ...
            for (unsigned t = 1; t < T; ++t) {          // t represents current time step
                for (unsigned s = 0; s < N; ++s) {      // s represents current state
                    ...
                    for (unsigned j = 0; j < N; ++j) {  // j represents previous state
                        ...
                        double current_transition_prob = transition_matrix(j, s);
                        double current_observation_prob;
                        double previous_viterbi_prob = viterbi(j, t - 1);
                        ...
                        if (get_word_index(observations[t]) == -1) {
                            ...
                            // And again, handle zero probs in unsmoothed models, else use actual probability
                            if (current_transition_prob == 0) {
                                maximum = std::max(maximum, previous_viterbi_prob * UNSEENTRANSITIONPROB * UNSEENWORDPROB);
                            } else {
                                maximum = std::max(maximum, previous_viterbi_prob * current_transition_prob * UNSEENWORDPROB);
                            }
                        }
                            ...
                            // Check for all possible zero probability combinations (will only occur if the model is unsmoothed) 
                            if (previous_viterbi_prob == 0 && current_transition_prob == 0 && current_observation_prob == 0) {
                                maximum = std::max(maximum, UNSEENTRANSITIONPROB * UNSEENTRANSITIONPROB * UNSEENTRANSITIONPROB);
                            }
                            else if (previous_viterbi_prob == 0 && current_transition_prob == 0 && current_observation_prob != 0) {
                                maximum = std::max(maximum, UNSEENTRANSITIONPROB * UNSEENTRANSITIONPROB * current_observation_prob);
                            }
                            else if (previous_viterbi_prob != 0 && current_transition_prob == 0 && current_observation_prob == 0) {
                                maximum = std::max(maximum, previous_viterbi_prob * UNSEENTRANSITIONPROB * UNSEENTRANSITIONPROB);
                            }
                            else if (previous_viterbi_prob == 0 && current_transition_prob != 0 && current_observation_prob == 0) {
                                maximum = std::max(maximum, UNSEENTRANSITIONPROB * current_transition_prob * UNSEENTRANSITIONPROB);
                            }
                            else if (previous_viterbi_prob != 0 && current_transition_prob == 0 && current_observation_prob != 0) {
                                maximum = std::max(maximum, previous_viterbi_prob * UNSEENTRANSITIONPROB * current_observation_prob);
                            }
                            else if (previous_viterbi_prob == 0 && current_transition_prob != 0 && current_observation_prob != 0) {
                                maximum = std::max(maximum, UNSEENTRANSITIONPROB * current_transition_prob * current_observation_prob);
                            }
                            else if (previous_viterbi_prob != 0 && current_transition_prob != 0 && current_observation_prob == 0) {
                                maximum = std::max(maximum, previous_viterbi_prob * current_transition_prob * UNSEENTRANSITIONPROB);
                            }

                            else {  // No zero probs, use actual probabilities
                                maximum = std::max(maximum, previous_viterbi_prob * current_transition_prob * current_observation_prob);
                            }
                        }

                        // Write calculated value into current viterbi trellis cell.
                        viterbi(s, t) = maximum;
                        
                        // Saves the current probability (necessary for filling in the backpointer trellis)
                        double current_prob = 0;

                        // Check probability again for unsmoothed models
                        if (transition_matrix(j, s) == 0) {
                            current_prob = viterbi(j, t - 1) * UNSEENTRANSITIONPROB;
                        } else {
                            current_prob = viterbi(j, t - 1) * transition_matrix(j, s);
                        }
    ...
    *************************************************/
    ...
    const std::string get_string_repr(const StateVector& vec) const {
        ...
        for (auto it = vec.begin(); it != vec.end(); ++it) {
            ...
            else { // Else append all other hidden states to it:
                str += (" " + get_hidden_symbol(*it));
            }
        }
        return str;
